...
today_holidays = holidays.today()
print(y, m, d, ': 农历', year_str + shengxiao + '年', month_str, day_str, '节气：', jieqi_str, '节日：', today_holidays)
```
...
节日判断功能基于遍历查表，目前之添加了少部分名称较短且较常用的节日，有需要的话可以自行按需修改。
...
holidays.py 中提供了三个列表，包含了所有节日，可以通过修改这三个列表来添加或者删除节日。
...
* 格式：(判定函数, 节日名)
  ...
  * 在编写判定函数时，内部提供了两个辅助函数 `_nth_weekday_of_month` 和 `_month_length` 分别返回某年某月第几个星期几的共农历日期和某公历年月的天数，具体使用方法请参考源代码及注释。
